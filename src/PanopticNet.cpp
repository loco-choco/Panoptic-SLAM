 ...
 * Co-authors: Vivian Suzano Medeiros, Marco Antonio Meggiolaro and Claudio
 * Semini Please report suggestions and comments to gabriel.fischer@iit.it
 */
...
PanopticNet::PanopticNet() {
  ...
  this->py_module =
      PyImport_ImportModule("panoptic_python"); // config_path.c_str()
  ...
  this->py_class =
      PyObject_GetAttrString(this->py_module, (char *)"Panoptic_FPN_Net");
  ...
  if (this->py_class && PyCallable_Check(this->py_class)) {
    ...
    this->py_instance =
        PyEval_CallObject(this->py_class, pargs); // call class __init__
    ...
    this->py_method =
        PyObject_GetAttrString(this->py_instance, (char *)"panoptic_run_cpp");
    ...
    this->py_get_image =
        PyObject_GetAttrString(this->py_instance, (char *)"get_output_img");
    ...
    this->py_get_instance_mask = PyObject_GetAttrString(
        this->py_instance, (char *)"get_all_instance_mask");
    ...
    this->py_get_all_masks =
        PyObject_GetAttrString(this->py_instance, (char *)"get_all_masks");
  ...
  // Release python thread lock
  this->py_lock = PyEval_SaveThread();
  //
}
...
//==========================================================
void PanopticNet::PyNumpy_AsCV(PyObject *pData, int rows, int cols,
                               cv::Mat &mask) {
...
Panoptic_image PanopticNet::GetPanoptic() {
  ...
  uchar *m = image.data;
  npy_intp mdim[3] = {image.rows, image.cols,
                      image.channels()}; // the dimensions of the matrix

  const int ND = 3;

  PyObject *mat = PyArray_SimpleNewFromData(
      ND, mdim, NPY_UINT8,
      reinterpret_cast<void *>(m)); // convert the cv::Mat to numpy.array
  ...
  if (PyList_Check(pValue)) {
    ...
    for (Py_ssize_t i = 0; i < PyList_Size(pValue); i++) {
      ...
      for (Py_ssize_t j = 0; j < PyList_Size(value);
           j++) //[id,isThing,score,category_id,instance_id,area,mask,bbox]
      {
        PyObject *sub_value = PyList_GetItem(value, j);
        Py_INCREF(sub_value);
        switch (j) {
        case Element::id_elem: {
          id = PyLong_AsLong(sub_value);
          break;
        }
        case Element::isThing_elem: {
          isThing = PyLong_AsLong(sub_value);
          break;
        }
        case Element::score_elem: {
          score = PyFloat_AsDouble(sub_value);
          break;
        }
        case Element::category_elem: {
          category_id = PyLong_AsLong(sub_value);
          break;
        }
        case Element::instance_elem: {
          instance_id = PyLong_AsLong(sub_value);
          break;
        }
        case Element::area_elem: {
          area = PyLong_AsLong(sub_value);
          break;
        }
        case Element::mask_elem: {
          PyNumpy_AsCV(sub_value, 480, 640, mask);
          break;
        }
        case Element::bbox_elem: {
          bbox = PyList_AsRect(sub_value);
          break;
        }
        }
        Py_DECREF(sub_value);
    ...
    std::cout << "Panoptic result is not a list" << std::endl;
  }
...
void PanopticNet::check_bbox(cv::Rect &bbox, int w, int h) {
  // std::cout<<"Input x "<< bbox.x << " y "<< bbox.y <<" width "<< bbox.width
  // << " height " << bbox.height << "\n";

  if ((bbox.x + bbox.width) > w) {
    bbox.width -= abs(w - bbox.x - bbox.width);
  }
  if ((bbox.y + bbox.height) > h) {
    bbox.height -= abs(h - bbox.y - bbox.height);
  }
...
//=========================================================
void PanopticNet::ShortTerm_DA(std::vector<Panoptic_Object> &objs,
...
PanopticNet::~PanopticNet() {
  ...
  PyEval_RestoreThread(this->py_lock);
  Py_Finalize();
}
...
void PanopticNet::SetTracker(Tracking *pTracker) { mpTracker = pTracker; }
...
Panoptic_image PanopticNet::GetResults() { return CurrentPanopticImg; }
